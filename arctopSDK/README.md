...
* Generative AI applications to improve foundation models and add empathy and responsiveness to individual preferences in the moment.
...
* Adaptive audio applications that tailor sound to user mood.
...
After the one-time calibration, real-time cognition metrics for endless usage are unlocked. For more information about calibration, see the section titled: [Verify a User is Calibrated for Arctop](#2-verify-a-user-is-calibrated-for-arctop) within this file.
...
2. [Arctop Permissions (Android Only)](#2-permissions-android-only)
3. [Bind to Service (Android Only)](#3-binding-to-the-service-android-only)
...
1. [Verify That a User is Logged In](#1-verify-a-user-is-logged-in)
2. [Verify That a User Has Been Calibrated for Arctop](#2-verify-a-user-is-calibrated-for-arctop)
...
#### 3. Binding to the Service (Android Only)
<details>
<summary>Android</summary>
...
You need an API Key to use the Arctop SDK in your app (see [Prerequisites](#1-prerequisites)). Once you have your API key and are ready to start working with the service, you will need to initialize the Arctop SDK.
...
#### 2. Verify a User is Calibrated for Arctop
...
* During calibration:
    * Headwear sensors are very sensitive to motion so during calibration try to move as little as possible.
    ...
    * Moving around too much in calibration will impact results so complete the session in the same location.
